...
def load_UIPRMD_data(ex = 1):
        
    train_path=current_file_dirpath / Path(f"../UIPRMD_pickle/EX_{str(ex)}/train_dataset.pkl")
    test_path=current_file_dirpath / Path(f"../UIPRMD_pickle/EX_{str(ex)}/train_dataset.pkl")
    
    Train = pickle.load(open(train_path, "rb"))
    Test = pickle.load(open(test_path, "rb"))
    
    print("Loading UIPRMD Dataset")
    return Train, Test


class UIPRMDConfig():
    def __init__(self):
        self.frame_l = 150  # the length of frames
        self.joint_n = 39  # the number of joints
